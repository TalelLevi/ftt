...
class Optimizer(abc.ABC):
    """
    a Basic template class for optimizers.
    """
...
#             # ====== YOUR CODE: ======
#             past_v = self.past_v[p] if p in self.past_v.keys() else 0
#             curr_v = self.momentum * past_v - self.learn_rate * (dp + self.reg * p)
#             p += curr_v
#             self.past_v[p] = curr_v
#             # ========================
#
#
...
#             # parameters tensor.
#             # ====== YOUR CODE: ======
#             past_r = self.past_r[p] if p in self.past_r.keys() else 0
#             curr_r = self.decay * past_r + (1 - self.decay) * ((dp + self.reg * p)**2)  # (dw)^2 element wise
#             p -= torch.div((self.learn_rate * (dp + self.reg * p)), torch.sqrt(curr_r + self.eps))
#             self.past_r[p] = curr_r
#             # ========================
